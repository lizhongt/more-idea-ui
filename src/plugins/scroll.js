...
GeminiScrollbar.prototype.update = function update() {
  ...
  let _originBarWidth = getScrollbarWidth()

  this._viewElement.style.width =
    (this.element.offsetWidth + _originBarWidth).toString() + 'px'
  this._viewElement.style.height =
    (this.element.offsetHeight + _originBarWidth).toString() + 'px'
...
GeminiScrollbar.prototype._bindEvents = function _bindEvents() {
  ...
  if (!isMobile()) {
    ...
    this._cache.events.touchStartHandler = this._touchStartHandler.bind(this)
    this._cache.events.touchEndHandler = this._touchEndHandler.bind(this)
  }
  ...
  if (!isMobile()) {
    ...
    this.element.addEventListener(
      'touchstart',
      this._cache.events.touchStartHandler
    )
    this.element.addEventListener(
      'touchend',
      this._cache.events.touchEndHandler
    )
  }
...
GeminiScrollbar.prototype._scrollHandler = function _scrollHandler() {
  var x =
    (this._viewElement.scrollLeft * this._trackLeftMax) / this._scrollLeftMax ||
    0
  var y =
    (this._viewElement.scrollTop * this._trackTopMax) / this._scrollTopMax || 0
...
GeminiScrollbar.prototype._touchStartHandler = function _touchStartHandler() {
  addClass(this.element, [CLASSNAMES.mobile])
}
GeminiScrollbar.prototype._touchEndHandler = function _touchEndHandler() {
  removeClass(this.element, [CLASSNAMES.mobile])
}
GeminiScrollbar.prototype._clickHorizontalThumbHandler = function _clickHorizontalThumbHandler(
  e
) {
...
GeminiScrollbar.prototype._startDrag = function _startDrag() {
...
GeminiScrollbar.prototype._mouseMoveDocumentHandler = function _mouseMoveDocumentHandler(
  ...
  if (this._prevPageY) {
    ...
    this._viewElement.scrollTop =
      (this._scrollTopMax * (offset - thumbClickPosition)) / this._trackTopMax
  ...
  if (this._prevPageX) {
    offset =
      e.clientX - this._scrollbarHorizontalElement.getBoundingClientRect().left
    thumbClickPosition = this._thumbSizeX - this._prevPageX

    this._viewElement.scrollLeft =
      (this._scrollLeftMax * (offset - thumbClickPosition)) / this._trackLeftMax
  }
}
